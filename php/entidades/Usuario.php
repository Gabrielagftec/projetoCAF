...
class Usuario {
    ...
    public $minutos_orcamento; //atributo auxiliar para aba de tarefas simplificadas;
    ...
    function __construct() {
        ...
        $this->minutos_orcamento = 0;
        $this->crc = "";

    }

    public function aceitarSolicitacao($con,$s){

        $ps = $con->getConexao()->prepare("UPDATE solicitacao_permissao_preco SET aceita=1 WHERE id_produto=$s->id_produto AND id_usuario=$s->id_usuario");
        $ps->execute();
        $ps->close();

    }

    public function getSolicitacoesLiberacao($con){

        if(strpos($this->nome, 'Elias') === false){
            return array();
        }

        $sols = array();

        $ps = $con->getConexao()->prepare("SELECT id_produto,id_usuario,observacao FROM solicitacao_permissao_preco WHERE aceita=0");
        $ps->execute();
        $ps->bind_result($id_produto,$id_usuario,$observacao);
        while($ps->fetch()){

            $s = new stdClass();
            $s->id_produto = $id_produto;
        ...
        $ps = $con->getConexao()->prepare("UPDATE solicitacao_permissao_preco SET aceita=-1 WHERE aceita=0");
        $ps->execute();
        $ps->close();
    ...
    public function dispensar($con){


        $ps = $con->getConexao()->prepare("UPDATE usuario SET excluido=1, dispensado=1 WHERE cpf='".$this->cpf->valor."'");
        $ps->execute();
        $ps->close();

        $html = Sistema::getHtml('email_rh_informa_aviso_previo', $this);
        $this->empresa->email->enviarEmail($this->email, "Aviso Previo", $html);
    ...
    public function getFilaSuporte($con){
        
        $ps = $con->getConexao()->prepare("SELECT id FROM fila_chat WHERE ABS(UNIX_TIMESTAMP(CURRENT_TIMESTAMP)-UNIX_TIMESTAMP(ultima_atualizacao)) < ".Suporte::$TEMPO_FILA." AND id_usuario=".$this->id);
        $ps->execute();
        $ps->bind_result($id);
        if($ps->fetch()){
            $ps->close();
            ...
            $ps = $con->getConexao()->prepare("SELECT id_usuario,usuario.nome FROM fila_chat INNER JOIN usuario ON usuario.id=fila_chat.id_usuario WHERE ABS(UNIX_TIMESTAMP(CURRENT_TIMESTAMP)-UNIX_TIMESTAMP(ultima_atualizacao)) < ".Suporte::$TEMPO_FILA." AND fila_chat.id<$id");
            $ps->execute();
    ...
    public function getSuporte($con,$criar=0){
       ...
       $suportes = array();
        ...
        if(count($suportes) === 0 && $criar>0){
            
            $s = new Suporte();
            $s->usuario = $this;
            if($s->atribuir($con,$criar>1)){
            ...
            foreach($suportes as $key=>$value){
             
                $ps = $con->getConexao()->prepare("SELECT id FROM expediente "
                        . "WHERE (DAYOFWEEK(CURRENT_DATE)-1)=dia_semana AND HOUR(CURRENT_TIMESTAMP)>=inicio AND HOUR(CURRENT_TIMESTAMP)<=fim AND id_usuario=".$value->atendente->id);
                $ps->execute();
                $ps->bind_result($tmp);
                $tem = $ps->fetch();
                $ps->close();
            ...
            if(count($suportes) === 0 && $criar>=1){
                
                return $this->getSuporte($con,$criar);
    ...
    public function getTarefasSolicitadas($con) {
        ...
        if ($cache === null) {
            ...
            $ps = $con->getConexao()->prepare("SELECT empresa.id,usuario.id,empresa.tipo_empresa FROM tarefa "
                    ...
                    . "WHERE (tarefa.id_usuario IN $cache->usuarios OR tarefa.criada_por=$this->id) AND usuario.excluido=false");
            $ps->execute();
        ...
        $sql = "SELECT "
                ...
                . "WHERE tarefa.excluida=false AND (tarefa.id_usuario IN $cache->associados OR tarefa.criada_por=$this->id) AND (tarefa.porcentagem_conclusao<100 OR tarefa.inicio_minimo < DATE_SUB(CURRENT_DATE, INTERVAL 7 DAY)) ORDER BY tarefa.porcentagem_conclusao<100 DESC, tarefa.prioridade DESC";
   ...
   public function getTodasTarefas($con, $filtro = "", $ordem = "", $agendadas = false) {

        $tipos_tarefa = array();
        
        $sql = "SELECT e.id,e.tipo_empresa FROM empresa e INNER JOIN usuario u ON u.id_empresa=e.id AND u.cpf='".$this->cpf->valor."' AND u.excluido=false GROUP BY e.id";
        $ps = $con->getConexao()->prepare($sql);
        $ps->execute();
        $ps->bind_result($id,$tipo);
        while($ps->fetch()){
            $tipos_tarefa[$id] = Sistema::getEmpresa($tipo, $id);
        }
        $ps->close();
        
        foreach($tipos_tarefa as $key=>$value){
            $tipos_tarefa[$key] = $value->getTiposTarefa($con);
        }
        ...
        $sql = "SELECT "
                ...
                . "FROM tarefa "
                . "INNER JOIN usuario uu ON tarefa.id_usuario=uu.id "
                . "INNER JOIN empresa ee ON ee.id=uu.id_empresa "
                . "LEFT JOIN (SELECT * FROM observacao WHERE observacao.excluida = false) observacao ON tarefa.id=observacao.id_tarefa "
                . "LEFT JOIN usuario u ON u.id=tarefa.criada_por "
                . "WHERE tarefa.excluida=false AND " . (!$agendadas ? "(tarefa.agendamento IS NULL OR tarefa.agendamento<CURRENT_TIMESTAMP) AND " : "") . "uu.cpf='".$this->cpf->valor."'";
                //43609 43687  43883 44166 44211
        if ($filtro !== "") {
        ...
        $ps = $con->getConexao()->prepare($sql);
        $ps->execute();
        $ps->bind_result($id,$id_empresa,$tipo_empresa, $inicio_minimo, $start_usuario, $ordem, $porcentagem_conclusao, $tipo_entidade_relacionada, $id_entidade_relacionada, $titulo, $descricao, $intervalos_execucao, $realocavel, $id_tipo_tarefa, $prioridade, $id_observacao, $porcentagem_observacao, $momento_observacao, $observacao, $ass);
        while ($ps->fetch()) {
            ...
            if (!isset($tarefas[$id])) {
                ...
                $t->tipo_entidade_relacionada = $tipo_entidade_relacionada;
                $t->id_entidade_relacionada = $id_entidade_relacionada;
                $t->titulo = addslashes($titulo);
                $t->descricao = addslashes($descricao);
                $t->intervalos_execucao = $intervalos_execucao;
                $t->realocavel = $realocavel == 1;

                foreach ($tipos_tarefa[$id_empresa] as $key => $tipo) {
                    if ($tipo->id === $id_tipo_tarefa) {
                        $t->tipo_tarefa = $tipo;
                        break;
                    }
    ...
    public function setAusencias($con, $ausencias) {
        ...
        foreach ($ausencias as $key => $value) {
            ...
            $ps = $con->getConexao()->prepare("UPDATE ausencia SET id_usuario=$this->id,inicio=inicio,fim=fim WHERE id=$value->id");
            $ps->execute();
            $ps->close();
        }
    }
    ...
    public function merge($con) {
        ...
        if($this->crc != ""){
        	...
        	$ps = $con->getConexao()->prepare("UPDATE usuario SET responsavel_receita=1, crc='".addslashes($this->crc)."' WHERE id=$this->id");
        	$ps->execute();
        	$ps->close();
    ...
    public function temPermissao($p) {
        ...
        if(strtolower($this->cargo->nome) == "diretor"){
        ...
        $this->empresa->rtc = $this->empresa->getRTC($con);
