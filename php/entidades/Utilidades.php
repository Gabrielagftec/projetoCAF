...
class Utilidades {
    ...
    private static $OTM_EMP_1 = array("cargos_fixos"=>true,"tarefas_fixas"=>true,"permissoes_especiais"=>true,"rtc"=>true);
    private static $OTM_TRANSP_1 = array("tabela"=>true,"empresa"=>true);
    private static $OTM_CLI_1 = array("empresa"=>true);
    private static $OTM_USU_1 = array("empresa"=>true,"permissoes"=>true);
    private static $OTM_FOR_1 = array("empresa"=>true);
    
    private static $OTM = null;
    
    
    public static function getOTM(){
        
        if(self::$OTM == null){
            
            self::$OTM = array(
                    "Pedido" => array(
                        array("transportadora",self::$OTM_TRANSP_1),
                        array("empresa",self::$OTM_EMP_1),
                        array("cliente",self::$OTM_CLI_1),
                        array("usuario",self::$OTM_USU_1),
                        array("logistica",self::$OTM_EMP_1),
                    ),
                    "Nota" => array(
                        array("transportadora",self::$OTM_TRANSP_1),
                        array("empresa",self::$OTM_EMP_1),
                        array("cliente",self::$OTM_CLI_1),
                        array("usuario",self::$OTM_USU_1),
                        array("fornecedor",self::$OTM_FOR_1),
                    )
                );
    ...
    public static function removeAcentos($string, $slug = false) {
        ...
        if (is_string($string)) {
            ...
            if(isset($string->act_45852123))return;
            $string->act_45852123 = "removed";
    ...
    public static function toJson($object, $pilha = null,$objectLevel=0,$redutiva=array(),$k=0) {
        ...
        if (is_array($object)) {
            ...
            foreach ($object as $i => $val) {
                ...
                $str .= Utilidades::toJson($val, $pilha,$objectLevel,$redutiva,$k);
            }
        ...
        if($objectLevel == 0){
            ...
            if(isset($dr[get_class($object)])){
                
                $red = $dr[get_class($object)];
        ...
        foreach ($object as $atributo => $valor) {
            ...
            if(count($red)>0){
                ...
                foreach($red as $key=>$redut){
                    
                   if($k==count($redut)-1){
                       
                       if(isset($redut[$k][$atributo])){
                           
                           continue 2;
                           
                       }
                       
                   }else if($redut[$k] == $atributo){
                ...
                if(count($filtro) == 0){
                    ...
                    $str .= ",\"$atributo\":" . Utilidades::toJson($valor, $pilha,$objectLevel+1);
                    
                }else{
                    
                    $str .= ",\"$atributo\":" . Utilidades::toJson($valor, $pilha,$objectLevel+1,$filtro,$k+1);
