...
class Tabela {
    ...
    public function merge($con) {
        ...
        if ($this->id == 0) {
            ...
            $ps = $con->getConexao()->prepare("INSERT INTO tabela(nome,excluida,prazo) VALUES('" . addslashes($this->nome) . "',false,$this->prazo)");
            $ps->execute();
            $this->id = $ps->insert_id;
            $ps->close();
        } else {

            $ps = $con->getConexao()->prepare("UPDATE tabela SET nome = '" . addslashes($this->nome) . "', excluida=false,prazo=$this->prazo WHERE id = " . $this->id);
            $ps->execute();
            $ps->close();
        }
