...
class CotacaoEntrada {
    ...
    public function merge($con) {
        ...
        if ($this->id == 0) {
            ...
            $ps = $con->getConexao()->prepare("INSERT INTO cotacao_entrada(id_fornecedor,frete,id_status,excluida,id_usuario,id_empresa,data,tratar_em_litros,observacao,local_entrega) VALUES(" . $this->fornecedor->id . ",$this->frete," . $this->status->id . ",false," . $this->usuario->id . "," . $this->empresa->id . ",FROM_UNIXTIME($this->data)," . ($this->tratar_em_litros ? "true" : "false") . ",'".addslashes($this->observacao)."',$this->local_entrega)");
            $ps->execute();
            $this->id = $ps->insert_id;
            $ps->close();
        } else {

            $ps = $con->getConexao()->prepare("UPDATE cotacao_entrada SET id_fornecedor=" . $this->fornecedor->id . ",frete=$this->frete,id_status=" . $this->status->id . ",excluida=false,id_usuario=" . $this->usuario->id . ",id_empresa=" . $this->empresa->id . ",data=FROM_UNIXTIME($this->data), tratar_em_litros=" . ($this->tratar_em_litros ? "true" : "false") . ",observacao='".addslashes($this->observacao)."',local_entrega=$this->local_entrega WHERE id = $this->id");
            $ps->execute();
            $ps->close();
        }
        ...
        if ($this->status->envia_email && $this->enviar_email) {
            try {
                ...
                $this->empresa->email->enviarEmail(array($this->fornecedor->email->filtro(Email::$VENDAS),$this->empresa->email->filtro(Email::$COMPRAS)), "Cotacao de produtos", $html);
