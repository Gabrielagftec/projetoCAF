...
class ProdutoPedidoEntrada {
    ...
    public function merge($con) {
        ...
        if ($this->id == 0) {
            ...
            $ps = $con->getConexao()->prepare("INSERT INTO produto_pedido_entrada(id_produto,quantidade,id_pedido,influencia_estoque,influencia_transito,valor,validade) VALUES(" . $this->produto->id . "," . $this->quantidade . "," . $this->pedido->id . ",$this->influencia_estoque,$this->influencia_transito,$this->valor,FROM_UNIXTIME($this->validade/1000))");
            $ps->execute();
            $this->id = $ps->insert_id;
            $ps->close();
        } else {

            $ps = $con->getConexao()->prepare("UPDATE produto_pedido_entrada SET id_produto=" . $this->produto->id . ",quantidade=" . $this->quantidade . ",id_pedido=" . $this->pedido->id . ",influencia_estoque=$this->influencia_estoque,influencia_transito=$this->influencia_transito, valor=$this->valor,validade=FROM_UNIXTIME($this->validade/1000) WHERE id=$this->id");
            $ps->execute();
            $ps->close();
        }
    }
