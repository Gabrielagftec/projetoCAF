...
class GrupoCidades {
    ...
    public function merge($con) {
        ...
        if ($this->id == 0) {
            ...
            $ps = $con->getConexao()->prepare("INSERT INTO grupo_cidades(nome,id_empresa,excluido,prazo) VALUES('" . addslashes($this->nome) . "',".$this->empresa->id.",false,$this->prazo)");
            $ps->execute();
            $this->id = $ps->insert_id;
            $ps->close();
        } else {

            $ps = $con->getConexao()->prepare("UPDATE grupo_cidades SET nome = '" . addslashes($this->nome) . "', id_empresa=".$this->empresa->id.", excluido = false,prazo=$this->prazo WHERE id = " . $this->id);
            $ps->execute();
            $ps->close();
        }
