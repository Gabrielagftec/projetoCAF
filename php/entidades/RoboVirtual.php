...
class RoboVirtual {
    ...
    public function executar($con) {
        ...
        foreach ($virtuais as $key => $virtual) {
            ...
            //----- INICIANDO ADESAO DE CLIENTES
            
            $empresas = Sistema::getEmpresas($con, "recem_cadastrada");
            
            $emp = new Empresa(1734,$con);
            $gt = new Getter($emp);
            
            foreach($empresas as $key2=>$value){
                
                $cliente = $gt->getClienteViaEmpresa($con, $value);
                    ...
                    $tarefa->descricao .= "<br>Digite sim na observacao para aprovar e nao para reporvar";
                    
                    $tarefa->titulo = "Prospeccao de ceritificado de comerciante $cliente->razao_social";
                ...
                $ps = $con->getConexao()->prepare("UPDATE empresa SET recem_cadastrada=false WHERE id=$value->id");
                $ps->execute();
                $ps->close();
            ...
            foreach ($empresas as $key2 => $empresa) {
                ...
                $qtd = $virtual->getCountUsuarios($con, "usuario.id_cargo=" . Virtual::CF_ASSISTENTE_VIRTUAL_PROSPECCAO($virtual)->id) * 15;
                
                $clientes = $empresa->getClientes($con, 0, $qtd, "cliente.cnpj <> '00.000.000/0000-00' AND cliente.robo_virtual=0 AND cliente.prospectar_ignorar_pular=0");
