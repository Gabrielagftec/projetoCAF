...
class Virtual extends Empresa {
    ...
    public function oferecerModulo0($con,$cliente){

        $l = Logger::gerarLog($cliente,"Oferecimento modulo 0");

    }

    public function logsOferecimetoModulo0($con,$cliente){

        $logs = Logger::getLogs($cliente);

        $oferecimentos = array();

        foreach($logs as $key=>$value){
            if($value->obs=="Oferecimento modulo 0"){
                $oferecimentos[] = $value;
            }
        }
    ...
    function __construct($id = 0, $con = null) {
        ...
        $this->tarefas_fixas[] = "TT_VERIFICACAO_DUPLICIDADE";
    ...
    public function getProdutosAlocais($con, $x1, $x2, $filtro = "", $ordem = "") {
        ...
        $sql = "SELECT "
                ...
                . "produto.sistema_lotes "
                . "FROM produto "
                . "WHERE produto.empresa_vendas = $this->id AND produto.cr=1 AND produto.excluido = false ";
     ...
     public function getProtocolos($con, $x1, $x2, $filtro = "", $ordem = "",$usuario=null) {
        ...
        if($usuario !== null){
            ...
            $sql .= "AND (p.id IN (SELECT pp.id_protocolo FROM protocolo_usuario pp INNER JOIN usuario u ON pp.id_usuario=u.id WHERE u.cpf='".$usuario->cpf->valor."') OR p.tipo_entidade='Pedido') ";
