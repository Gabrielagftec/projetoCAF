...
class ProdutoEncomenda {
    ...
    public function atualizarCustos() {
        ...
        $this->ipi_final = ($this->valor_base_final + $this->juros_final + $this->icms_final) * ($this->produto->categoria->ipi / 100);
            ...
            foreach ($this->encomenda->produtos as $produto) {

                $total += $produto->valor_base_inicial * $produto->quantidade;
            }

            if ($total > 0) {

                $perc = ($this->valor_base_inicial * $this->quantidade) / $total;

                $this->frete_inicial = round((($this->encomenda->frete * $perc) / $this->quantidade), 2);
            }
            ...
            foreach ($this->encomenda->produtos as $produto) {
                ...
                $total += $produto->valor_base_final * $produto->quantidade;
            }

            if ($total > 0) {

                $perc = ($this->valor_base_final * $this->quantidade) / $total;

                $this->frete_final = round((($this->encomenda->frete * $perc) / $this->quantidade), 2);
            }
