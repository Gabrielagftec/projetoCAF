   ...
   public function aoFinalizar($tarefa, $usuario) {
        ...
        if (count($pt) === 2) {
            if ($pt[0] === "PED") {
                ...
                $log = Logger::gerarLog($pedido, "Rastreamento do pedido $pedido->id do cliente ".$pedido->cliente->razao_social.": ".$observ->observacao);

                $html = Sistema::getHtml('log',$log);

                $empresa->email->enviarEmail(array($empresa->email,$pedido->cliente->email),"Rastreamento",$html);
