...
class Encomenda {
    ...
    public function getProdutos($con) {
        ...
        $ps = $con->getConexao()->prepare("SELECT produto_encomenda.id,"
                ...
                . "produto_encomenda.frete_inicial,"
                . "produto_encomenda.frete_final,"
                . "produto.id,"
                . "produto.codigo,"
        ...
        while ($ps->fetch()) {
            ...
            $pp->id = $id;
            $pp->frete_inicial = $frete_inicial;
            $pp->frete_final = $frete_final;
    ...
    public function merge($con) {
        ...
        if ($this->id == 0) {
            ...
            $ps = $con->getConexao()->prepare("INSERT INTO encomenda(id_cliente,observacoes,id_empresa,data,excluida,id_usuario,prazo,parcelas,id_status,frete,id_transportadora) VALUES(" . $this->cliente->id . ",'$this->observacoes'," . $this->empresa->id . ",FROM_UNIXTIME($this->data/1000),false," . $this->usuario->id . ",$this->prazo,$this->parcelas," . $this->status->id . ",$this->frete,".$this->transportadora->id.")");
            $ps->execute();
            ...
            $ps = $con->getConexao()->prepare("UPDATE encomenda SET id_cliente=" . $this->cliente->id . ",observacoes='$this->observacoes',id_empresa=" . $this->empresa->id . ",data=FROM_UNIXTIME($this->data/1000),excluida=false,id_usuario=" . $this->usuario->id . ",prazo=$this->prazo,parcelas=$this->parcelas,id_status=" . $this->status->id . ",frete=$this->frete,id_transportadora=".$this->transportadora->id." WHERE id=$this->id");
            $ps->execute();
            $ps->close();
