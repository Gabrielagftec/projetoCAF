...
class Movimento {
    ...
    public function corrigirSaldo($con){
        ...
        $movimentos = array();
        $ps = $con->getConexao()->prepare("SELECT movimento.id,(movimento.valor+movimento.juros-movimento.descontos)*(CASE WHEN operacao.debito THEN -1 ELSE 1 END) FROM movimento"
    ...
    public function insert($con, $rp_insert = false) {
        ...
        if ($this->id == 0) {
            ...
            if ($ps->fetch()) {
                ...
                $this->data += ceil(rand(1, 1000))*1000;
