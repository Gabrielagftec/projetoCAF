...
class Pedido {
    ...
    public function gerarCobranca() {
        ...
        try {
            $this->empresa->email->enviarEmail(array($this->empresa->email->filtro(Email::$FINANCEIRO),$this->cliente->email->filtro(Email::$FINANCEIRO)), "Cobranca de pagamento", $log->toHtml());
        } catch (Exception $ex) {
    ...
    public function merge($con, $recursao = false) {
        ...
        if ($this->id == 0) {
            ...
            if ($this->prazo < 3) {
                ...
                if($this->cliente->getDividas($con,10)>10){
                                        
                    //throw new Exception('Atualize seu limite de credito com o nosso financeiro em ate 2 horas para que o pedido seja efetuado, caso nao seja feito ele sera cancelado automaticamente');
                ...
                if($this->cliente->getDividas($con,5)>10){
            ...
            $orcamento = $this->status->id === 11;

            if ($analisar_credito && !$orcamento) {
                $t = new Tarefa();
                ...
                $ps->execute();
                $ps->close();
            } else if ($passar_direto && !$orcamento) {

                $tem_cancelamento = 0;
                $ps = $con->getConexao()->prepare("SELECT id_status=10 FROM pedido WHERE id_cliente=".$this->cliente->id." AND DATE_ADD(pedido.data,INTERVAL 3 MONTH) >= CURRENT_DATE ORDER BY pedido.id DESC");
                $ps->execute();
                $ps->bind_result($idst);
                while($ps->fetch() && $tem_cancelamento < 20){
                    if($idst==1)break;
                    $tem_cancelamento++;
                }
                $ps->close();

                $tem_cancelamento = $tem_cancelamento<20;
                
                $tem_limite = $this->cliente->getLimiteCredito() > 0;

                if ($this->prazo < 3 && ($tem_cancelamento || !$tem_limite)) {
                    $t = new Tarefa();
            ...
            //------------ INICIA?O PROTOCOLO VALIDADE ----
            
            if(!$orcamento){

	            $validades_curtas = array();
	            
	            $data_validade_curta = (round(microtime(true))*1000)+(Sistema::getMesesValidadeCurta()*30*24*60*60*1000);
	            
	            foreach($this->produtos as $key=>$value){
	                
	                $p = $value->produto;
	                
	                if($p->sistema_lotes && $value->validade_minima>1000){
	                    
	                    if($value->validade_minima < $data_validade_curta){
	                        
	                        $validades_curtas[] = $value;
	            ...
	            if(count($validades_curtas) > 0){
	                
	                $ps = $con->getConexao()->prepare("SELECT empresa_vendas FROM empresa WHERE empresa_vendas>0 AND id=".$this->empresa->id);
	                $ps->execute();
	                $ps->bind_result($virt);
	                if($ps->fetch()){
	                    ...
	                    $virtual = new Virtual($virt, $con);
	                    
	                    $tipo_protocolo = null;
	                    
	                    $tipos_protocolo = $virtual->getTiposProtocolo($con);
	                    
	                    foreach($tipos_protocolo as $k1=>$tipo){
	                        if($tipo_protocolo === null){
	                            $tipo_protocolo = $tipo;
	                        }else{
	                            if($tipo->prioridade > $tipo_protocolo->prioridade){
	                                $tipo_protocolo = $tipo;
	                            }
	                        }
	                    }
	                    ...
	                    if($tipo_protocolo !== null){
	                        ...
	                        $p = new Protocolo();
	                        $p->descricao = "Pedido $this->id e prioritario, contem produtos de validade curta: ";
	                        
	                        foreach($validades_curtas as $vk=>$vc){
	                            
	                            $p->descricao .= " ".$vc->produto->nome.", QTD: ".$vc->quantidade.", Validade: ".date("d/m/Y",$vc->validade_minima/1000);
	                            
	                        }
	                        
	                        $p->titulo = "Pedido $this->id cotem produtos de validade curta";
	                        
                                $empp = new Empresa(9999);
	                        ...
	                        $cargos = array(
	                            Virtual::CF_ASSISTENTE_VIRTUAL_SUPORTE($virtual),
                                    Logistica::CF_SEPARADOR($empp),
                                    Logistica::CF_SUPERVISOR_LOGISTICA($empp),
                                    Logistica::CF_ESTAGIARIO_LOGISTICA($empp),
                                    Logistica::CF_ENCARREGADO_LOGISTICA($empp),
                                    Logistica::CF_AUXILIAR_ADM($empp),
                                    Logistica::CF_COORDENADOR_LOGISTICA($empp)
	                        );
	                        
	                        $in_cargos = "(-1";
	                        
	                        foreach($cargos as $k1=>$c1){
	                            $in_cargos .= ",$c1->id";
	                        }
	                        
	                        $in_cargos .= ")";
	                        
	                        $usuarios = $virtual->getUsuarios($con, 0, 5,"usuario.id_cargo IN $in_cargos");
	                        
                                if($this->logistica !== null){
                                    
                                    $lus = $this->logistica->getUsuarios($con, 0, 5,"usuario.id_cargo IN $in_cargos");
                                    
                                    foreach($lus as $ku=>$us){
                                        $usuarios[] = $us;
                                    }
	                        ...
	                        $p->tipo = $tipo_protocolo;
	                        
	                        $p->tipo_entidade = "Pedido";
	                        
	                        $p->id_entidade = $this->id;
        ...
        if($us !== null){
            ...
            if(Sistema::isBoasVindas($us, $con)){
                ...
                $html = Sistema::getHtml('boas_vindas', $p);
                
                $this->empresa->email->enviarEmail($this->cliente->email,"Boas Vindas",$html);
    ...
    public function delete($con) {
        ...
        $ps = $con->getConexao()->prepare("DELETE FROM tarefa "
                . "WHERE tipo_entidade_relacionada='PED_" . $this->empresa->id . "' AND id_entidade_relacionada=$this->id AND porcentagem_conclusao<100");
        $ps->execute();
        $ps->close();
