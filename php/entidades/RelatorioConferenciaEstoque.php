...
class RelatorioConferenciaEstoque extends Relatorio {
    ...
    public function __construct($empresa = null) {
        ...
        if($empresa->tipo_empresa !== 1){
                    parent::__construct("SELECT p.codigo as 'codigo',p.nome as 'nome',p.local as 'local',IFNULL(i.quantidade,0) as 'estoque_anterior',k.saidas as 'saidas',k.entradas as 'entradas',(IFNULL(i.quantidade,0)-k.saidas+k.entradas) as 'atual',DATE_FORMAT(k.data,'%d/%m/%Y') as 'data' FROM 
            ...
            (SELECT IFNULL(entradas.id_produto,saidas.id_produto) as 'id_produto', IFNULL(entradas.qtd,0) as 'entradas', IFNULL(saidas.qtd,0) as 'saidas',IFNULL(entradas.data,saidas.data) as 'data' FROM
             ...
             LEFT JOIN inventario i ON i.id_produto = p.id AND i.data=k.data
