...
class RelatorioInventario extends Relatorio {
    ...
    public function getObservacoes($empresa) {
        ...
        $sql = "SELECT SUM(total),SUM(total_liquido),SUM(icms_recuperado) FROM (SELECT * FROM inventario GROUP BY data,id_produto) i INNER JOIN produto p ON i.id_produto=p.id LEFT JOIN empresa l ON p.id_logistica=l.id WHERE i.id_empresa=$empresa->id AND p.id_categoria=1164";
        ...
        if($ps->fetch()){
            return "Referente a :".date("d/m/Y",$this->campos[0]->inicio/1000)." 23:59:59; Parcial: R$ $total; Icms Recuperado: R$ $icms ;Total Liquido: R$ $total_liquido";
        }
        $ps->close();
        return "";
