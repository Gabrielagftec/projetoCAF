var projeto = "https://interno.rtcagro.com.br";
        ...
        attList: function () {
            ...
            var este = this;
                ...
                lista.getElementos(este.pagina * (este.por_pagina * este.por_coluna),
                        (este.pagina + 1) * (este.por_pagina * este.por_coluna),
                        este.filtro, este.ordem, function (e) {
                           
                            este.pagina_mu = este.pagina+1;
                
                            este.quantidade = e.qtd;

                            var np = Math.ceil(e.qtd / (este.por_pagina * este.por_coluna));
        ...
        attList: function () {
            ...
            if (comparator != null) {
                for (var i = 1; i < lista.length; i++) {
                    for (var j = i; j > 0 && comparator(this.listaTotal[j], this.listaTotal[j - 1]); j--) {
                        var k = this.listaTotal[j];
                        this.listaTotal[j] = this.listaTotal[j - 1];
                        this.listaTotal[j - 1] = k;
                    }
                }
            }
            ...
            lbl:
                ...
                for (a in this.listaTotal[i]) {

                    if (typeof this.listaTotal[i][a] === 'string') {
                        if (this.listaTotal[i][a].toUpperCase().indexOf(this.filtro.toUpperCase()) >= 0) {

                            lst[lst.length] = this.listaTotal[i];
                            continue lbl;
            ...
            if (s) {
                ...
                if ("" == l) {
                    for (var o = n.split(" "), A = 1; A < o.length; A++) {
                        var v = o[A].split("=", 2);
                        a[v[0]] = (v.length>1)?v[1].substr(1, v[1].length - 2):"";
                    }
                    l = n = o[0], e--, h = -1
                } else {
                    var u = n.length;
    ...
    var up = function (arquivo, obj, fn) {
        ...
        if (reader) {
            ...
            reader.onload = function () {
                ...
                obj.total += Math.ceil(bytes.length / buffer);
    ...
    return {
        ...
        link: function (scope, element, attrs) {
            ...
            var lk = function () {
                ...
                scope.meses_alias = ["Jan", "Fev", "Mar", "Abr", "Mai", "Jun", "Jul", "Ago", "Set", "Out", "Nov", "Dec"];
                ...
                scope.setIntervalo = function (dt) {
                    ...
                    if (scope.inicio_fim === 0) {

                        scope.inicio = dt.millis;

                        if(scope.fim < scope.inicio){
                            scope.fim = scope.inicio;
                        }
